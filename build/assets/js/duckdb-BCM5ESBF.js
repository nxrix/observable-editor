...
var mainWorkerUrl = bundle.then(({ mainWorker }) => {
...
var DuckDBClient = class _DuckDBClient {
  ...
  static async of(sources = {}, config = {}) {
    ...
    if (config.filesystem?.forceFullHTTPReads === void 0) {
...
Object.defineProperty(DuckDBClient, "name", { value: "DuckDBClient" });
...
async function insertSource(database, name, source) {
  ...
  if (isFlechetteTable(source))
    return insertFlechetteTable(database, name, source);
  ...
  if (source && typeof source === "object") {
    if ("data" in source) {
      ...
      if (isFlechetteTable(data))
        return insertFlechetteTable(database, name, data, options);
...
async function insertArqueroTable(database, name, source) {
  const buffer = typeof source.toArrowIPC === "function" ? source.toArrowIPC() : source.toArrowBuffer();
...
function isArqueroTable(value) {
  return value && (typeof value.toArrowBuffer === "function" || typeof value.toArrowIPC === "function");
}
function isFlechetteTable(value) {
  return value && typeof value.getChild === "function" && typeof value.toColumns === "function" && value.schema && Array.isArray(value.schema.fields);
}
