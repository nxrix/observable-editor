...
import {
  ...
  parseCode,
  parser as parser2
} from "./chunk-C5VPLTAF.js";
import {
  configureNesting,
  parser as parser3
} from "./chunk-VQDAS7HA.js";
import {
  parser as parser4
} from "./chunk-ISAVHIBI.js";
import "./chunk-OM32DVFN.js";
import {
  IterMode,
  NodeProp,
  NodeType,
  ...
  slot(addresses) {
    ...
    return {
      ...
      reconfigure: (state, oldState) => {
        ...
        if ((reInit = init2.find((i2) => i2.field == this)) && reInit != oldInit.find((i2) => i2.field == this)) {
...
function countColumn(string, tabSize, to = string.length) {
  let n = 0;
  for (let i2 = 0; i2 < to && i2 < string.length; ) {
    if (string.charCodeAt(i2) == 9) {
      n += tabSize - n % tabSize;
      i2++;
    } else {
      n++;
      i2 = findClusterBreak2(string, i2);
    }
  }
  return n;
}
...
function moveVisually(line, order2, dir, start, forward) {
  ...
  if (start.head == line.from && start.assoc < 0) {
    if (!forward)
      return null;
    startIndex = order2[spanI = 0].side(false, dir);
  } else if (start.head == line.to && start.assoc > 0) {
    if (forward)
      return null;
    startIndex = order2[spanI = order2.length - 1].side(true, dir);
  } else {
...
observers.compositionstart = observers.compositionupdate = (view) => {
  ...
  if (view.inputState.composing < 0) {
    let { main: main2 } = view.state.selection;
    if (!main2.empty && view.lineBlockAt(main2.from).from != view.lineBlockAt(main2.to).from) {
      view.dispatch({
...
var CachedOrder = class _CachedOrder {
  constructor(from, to, dir, isolates, fresh2, order2) {
    this.from = from;
    this.to = to;
    this.dir = dir;
...
var selectionLayer = /* @__PURE__ */ layer({
  above: false,
  markers(view) {
    let markers = [], { main: main2, ranges } = view.state.selection;
    for (let r of ranges)
      if (!r.empty) {
        for (let marker of RectangleMarker.forRange(view, "cm-selectionBackground", r))
          markers.push(marker);
      }
    if (browser.ios && !main2.empty && view.state.facet(selectionConfig).iosSelectionHandles) {
      for (let piece of RectangleMarker.forRange(view, "cm-selectionHandle cm-selectionHandle-start", EditorSelection.cursor(main2.from, 1)))
        markers.push(piece);
      for (let piece of RectangleMarker.forRange(view, "cm-selectionHandle cm-selectionHandle-end", EditorSelection.cursor(main2.to, 1)))
        markers.push(piece);
    }
    return markers;
  },
...
var Language = class {
  ...
  */
  constructor(data2, parser5, extraExtensions = [], name = "") {
    this.data = data2;
    this.name = name;
...
var LRLanguage = class _LRLanguage extends Language {
  constructor(data2, parser5, name) {
    super(data2, parser5, [], name);
    this.parser = parser5;
  }
  /**
  ...
  createListBox(options, id, range) {
    ...
    for (let i2 = range.from; i2 < range.to; i2++) {
      ...
      if (section) {
        ...
        if (name != curSection && (i2 > range.from || range.from == 0)) {
          curSection = name;
    ...
    for (let line of template.split(/\r\n?|\n/)) {
      while (m = /[#$]\{(?:(\d+)(?::([^{}]*))?|((?:\\[{}]|[^{}])*))\}/.exec(line)) {
        ...
        for (let i2 = 0; i2 < fields.length; i2++) {
          if (seq != null ? fields[i2].seq == seq : name ? fields[i2].name == name : false)
            found = i2;
        }
        if (found < 0) {
          let i2 = 0;
...
function renderDiagnostic(view, diagnostic, inPanel) {
  ...
  return crelt("li", { class: "cm-diagnostic cm-diagnostic-" + diagnostic.severity }, crelt("span", { class: "cm-diagnosticText" }, diagnostic.renderMessage ? diagnostic.renderMessage(view) : diagnostic.message), (_a2 = diagnostic.actions) === null || _a2 === void 0 ? void 0 : _a2.map((action, i2) => {
    ...
    let { name } = action, keyIndex = keys2[i2] ? name.indexOf(keys2[i2]) : -1;
    ...
    return crelt("button", {
      ...
      onclick: click,
      onmousedown: click,
      "aria-label": ` Action: ${name}${keyIndex < 0 ? "" : ` (access key "${keys2[i2]})"`}.`
    }, nameElt);
...
var autoCloseTags = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  ...
  let closeTags = state.changeByRange((range) => {
    ...
    if (state.doc.sliceString(head - 1, head) != text || around.name == "JSXAttributeValue" && around.to > head) ;
      ...
      if (base3 && empty2.from == head - 2 && ((name = elementName(state.doc, base3.firstChild, head)) || ((_a2 = base3.firstChild) === null || _a2 === void 0 ? void 0 : _a2.name) == "JSXFragmentTag")) {
        let insert2 = `${name}>`;
  ...
  if (node.name == "TagName") {
    for (let { parent } = node; parent; parent = parent.parent)
      if (parent.name == "Block")
        return { from: node.from, options: properties(), validFor: identifier };
    return { from: node.from, options: tags2, validFor: identifier };
  }
...
function isClosed(doc2, elt, name) {
  ...
  for (; ; ) {
    if (((_a2 = elt.lastChild) === null || _a2 === void 0 ? void 0 : _a2.name) != "CloseTag")
      return false;
    let next = elt.parent;
    if (!next || elementName2(doc2, next) != name)
      return true;
    elt = next;
  }
}
var autoCloseTags2 = /* @__PURE__ */ EditorView.inputHandler.of((view, from, to, text, insertTransaction) => {
  ...
  let closeTags = state.changeByRange((range) => {
    ...
    if (didType && text == ">" && after.name == "EndTag") {
      ...
      if ((name = elementName2(state.doc, tag.parent, head)) && !selfClosers.has(name) && !isClosed(state.doc, tag.parent, name)) {
      ...
      let tag = after.parent;
      if (after.from == head - 2 && ((_a2 = tag.lastChild) === null || _a2 === void 0 ? void 0 : _a2.name) != "CloseTag" && (name = elementName2(state.doc, tag, head)) && !selfClosers.has(name)) {
        let to2 = head + (state.doc.sliceString(head, head + 1) === ">" ? 1 : 0);
        let insert2 = `${name}>`;
        return {
...
var Context = class {
  ...
  marker(doc2, add2) {
    let number = this.node.name == "OrderedList" ? String(+itemNumber(this.item, doc2)[2] + add2) : "";
...
var pasteURLAsLink = /* @__PURE__ */ EditorView.domEventHandlers({
  paste: (event, view) => {
    ...
    if (/^www\./.test(link))
      link = "https://" + link;
    if (!markdownLanguage.isActiveAt(view.state, main2.from, 1))
      return false;
    ...
    tree.iterate({
      ...
      to: main2.to,
      enter: (node) => {
        if (node.from > main2.from || nonPlainText.test(node.name))
          crossesNode = true;
      },
      leave: (node) => {
        if (node.to < main2.to)
    ...
    if (crossesNode)
      return false;
    view.dispatch({
      changes: [{ from: main2.from, insert: "[" }, { from: main2.to, insert: `](${link})` }],
      userEvent: "input.paste",
      scrollIntoView: true
    });
    return true;
...
var freshNotebook = `<!doctype html>
<notebook theme="air">
...
var pendingOverlayResize = /* @__PURE__ */ new Map();
var order = [];
var nextId = 0;
var ready = false;
var fresh = false;
var queued = null;
var notebookTitle = "untitled";
var fileHandle = null;
var statusTimer;
var unsaved = false;
var channel = new MessageChannel();
var initialized = false;
var send = (msg) => channel.port1.postMessage(msg);
var pickerTypes = [{
  description: "Observable notebook",
  accept: { "text/html": [".html", ".htm"] }
}];
var isNotebook = (text) => /<notebook[\s>]/i.test(text);
...
var wraps = (mode) => mode !== "js" && mode !== "ts" && mode !== "ojs";
var createEditor = (cell, parent) => new EditorView({
  doc: cell.value,
  parent,
  extensions: [
    basicSetup,
...
var setStatus = (text) => {
  ...
  statusTimer = setTimeout(() => status.style.opacity = 0, 2e3);
};
var suggestedName = () => `${notebookTitle.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "untitled"}.html`;
var downloadNotebook = (html2) => {
  const a = document.createElement("a");
  a.href = URL.createObjectURL(new Blob([html2], { type: "text/html" }));
...
var saveNotebook = async (html2) => {
  try {
    if (!fileHandle) {
      if (!window.showSaveFilePicker) return downloadNotebook(html2);
    ...
    if (error.name === "NotAllowedError") {
      fileHandle = null;
      return saveNotebook(html2);
    }
    console.error(error);
    setStatus(`Save failed: ${error.message ?? error}`);
  }
};
var loadNotebook = async () => {
...
var sendOverlayResize = (id, value) => {
...
var overlayResizeScheduled = false;
var resizeWorkerScheduled = false;
var resizeWorker = () => {
...
var createOverlay = (id, cell) => {
  ...
  add2.textContent = "+";
  add2.style.width = "24px";
  add2.style.display = "flex";
  add2.style.alignItems = "center";
  add2.style.justifyContent = "center";
  add2.style.cursor = "pointer";
  add2.onclick = () => {
    const cell2 = {
      mode: "ojs",
      value: "",
  ...
  const expand = () => {
    ...
    remove2.style.fontFamily = "inherit";
    remove2.textContent = "Delete";
    remove2.onclick = () => {
      if (order.length > 1) {
        const index = order.indexOf(id);
        if (index !== -1) {
    ...
    toolbar.append(select, run, pin, hide, up, down, remove2, close);
...
var openNotebook = (html2, isFresh = false) => {
  fresh = isFresh;
  if (isFresh) {
    fileHandle = null;
...
document.addEventListener("drop", async (event) => {
  ...
  const file = files.find(
    (file2) => file2.type === "text/html" || /\.html?$/i.test(file2.name)
  );
  if (!file) return;
  const text = await file.text();
  if (!isNotebook(text)) return setStatus("Not an Observable notebook");
