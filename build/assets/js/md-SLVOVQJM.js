  ...
  for (let j = 0, l2 = blockTokens.length; j < l2; j++) {
    ...
    for (let i2 = tokens.length - 1; i2 >= 0; i2--) {
      ...
      if (currentToken.type === "text" && state.md.linkify.test(currentToken.content)) {
        ...
        replacements2.push({ index: i2, nodes });
      }
    }
    if (replacements2.length > 0) {
      let newTokensLength = tokens.length;
      for (const replacement of replacements2) {
      ...
      for (let i2 = 0; i2 < tokens.length; i2++) {
        const replacement = replacements2[replacementIndex];
        if (replacement?.index === i2) {
          for (const node of replacement.nodes) {
            newTokens[newTokenIndex++] = node;
          }
          replacementIndex++;
        } else {
          newTokens[newTokenIndex++] = tokens[i2];
        }
      }
...
function addReplacement(replacements2, tokenIdx, pos, ch) {
  if (!replacements2[tokenIdx]) {
    replacements2[tokenIdx] = [];
  }
  replacements2[tokenIdx].push({ pos, ch });
}
function applyReplacements(str, replacements2) {
  let result = "";
  let lastPos = 0;
  replacements2.sort((a2, b2) => a2.pos - b2.pos);
  for (let i2 = 0; i2 < replacements2.length; i2++) {
    const replacement = replacements2[i2];
...
function process_inlines(tokens, state) {
  ...
  for (let i2 = 0; i2 < tokens.length; i2++) {
    ...
    OUTER:
      while (pos < max) {
        ...
        if (!canOpen && !canClose) {
          if (isSingle) {
            addReplacement(replacements2, i2, t2.index, APOSTROPHE);
          }
          continue;
        }
        if (canClose) {
          ...
          if (j >= 0 && stack[j].level === thisLevel) {
            ...
            addReplacement(replacements2, i2, t2.index, closeQuote);
            addReplacement(replacements2, item.tokenIdx, item.contentPos, openQuote);
  ...
  Object.keys(replacements2).forEach(function(tokenIdx) {
    tokens[tokenIdx].content = applyReplacements(tokens[tokenIdx].content, replacements2[tokenIdx]);
  });
}
...
var f = o(function(e2, n2, t2, r2, a2) {
  ...
  r2.tokens[a2 + 1].children = i2;
});
Object.assign(f.defaults, { safariReaderFix: false });
var d = o(function(e2, r2, a2, i2, l2) {
  ...
  if (r2.class && f2.push(["class", r2.class]), f2.push(["href", r2.renderHref(e2, i2)]), f2.push.apply(f2, Object.entries(r2.renderAttrs(e2, i2))), "visually-hidden" === r2.style) {
    if (u2.push(Object.assign(new i2.Token("span_open", "span", 1), { attrs: [["class", r2.visuallyHiddenClass]] }), Object.assign(new i2.Token("text", "", 0), { content: r2.assistiveText(s2) }), new i2.Token("span_close", "span", -1)), r2.space) {
      ...
      u2[n[r2.placement]](Object.assign(new i2.Token("string" == typeof r2.space ? "html_inline" : "text", "", 0), { content: d2 }));
    }
  ...
  "aria-label" === r2.style ? f2.push(["aria-label", r2.assistiveText(s2)]) : ["aria-describedby", "aria-labelledby"].includes(r2.style) && f2.push([r2.style, e2]);
  var p2 = [Object.assign(new i2.Token("link_open", "a", 1), { attrs: c(f2) })].concat(u2, [new i2.Token("link_close", "a", -1)]);
...
Object.assign(d.defaults, { style: "visually-hidden", space: true, placement: "after", wrapper: null }), b.permalink = { __proto__: null, legacy: r, renderHref: a, renderAttrs: i, makePermalink: o, linkInsideHeader: s, ariaHidden: u, headerLink: f, linkAfterHeader: d }, b.defaults = { level: 1, slugify: function(e2) {
...
var slugify2 = slugifyWithCounter();
mi.use(b, {
  level: [2, 3],
  slugify: (s2) => slugify2(s2).replace(/^(?=\d)/, "-"),
...
var md = (template, ...values) => {
  ...
  if (codes.length > 0)
    import("./highlight-BT2VHD6J.js").then(({ highlight }) => codes.forEach(highlight));
